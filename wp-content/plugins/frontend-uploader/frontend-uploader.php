...
class Frontend_Uploader {
	...
	function upload_form( $atts, $content = null ) {
		...
		extract( shortcode_atts( array(
					'description' => '',
					'title' => __( 'ارفع ملفاتك', 'frontend-uploader' ),
					'type' => '',
					'class' => 'validate',
					'success_page' => '',
					'form_layout' => 'image',
...
<?php
		...
		$file_desc = __( 'الملفات', 'frontend-uploader' );
		$submit_button = __( 'حفظ', 'frontend-uploader' );
		...
		if ( !( isset( $this->settings['suppress_default_fields'] ) && 'on' === $this->settings['suppress_default_fields'] ) && ( $suppress_default_fields === false ) ) {
			...
			echo $this->shortcode_content_parser( array(
					...
					'id' => 'ug_post_title',
					'class' => 'required',
					'description' => __( 'اسم الملف', 'frontend-uploader' ),
				), null, 'input' );
			...
			case 'media':
				...
				echo $this->shortcode_content_parser( array(
						...
						'id' => 'ug_content',
						'class' => 'required',
						'description' => __( 'وصف الملف', 'frontend-uploader' ),
					), null, 'textarea' );
...
<?php
	...
	function _display_response_notices( $res = array() ) {
		...
		$map = apply_filters( 'fu_response_notices', array(
			'fu-sent' => array(
				'text' => __( 'تم رفع الملفات بنجاح .. من فضلك انتظر تأكيد الادارة!', 'frontend-uploader' ),
				'class' => 'success',
			),
			'fu-post-sent' => array(
				'text' => __( 'تم إرسال مشاركتك بنجاح!', 'frontend-uploader' ),
				'class' => 'success',
			),
			'fu-error' => array(
				'text' => __( 'حدث خطأ في التقديم الخاص بك', 'frontend-uploader' ),
				'class' => 'failure',
			),
			'fu-spam' => array(
				'text' => __( 'فشل إرسالك في فحص البريد العشوائي', 'frontend-uploader' ),
				'class' => 'failure',
			),
		) );
